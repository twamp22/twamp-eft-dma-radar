...
namespace eft_dma_shared.Common.Unity.LowLevel.Hooks
{
    public static class NativeHook
    {
        ...
        private static byte[] GetInvokeHookShellcode()
        {
            return new byte[]
            {
                0x48, 0x83, 0xEC, 0x38, 0x48, 0xB8, 0xFA, 0xFA,
        ...
        public static unsafe bool Initialize()
        {
            lock (SyncRoot)
            {
                ...
                try
                {
                    ...
                    // Disabled in read-only branch:
                    // Memory.WriteBufferEnsure(invokeHookFunction, invokeHookBytes.AsSpan());
                    LoneLogging.WriteLine("[NativeHook]: Skipping writing pre-invoke hook shellcode (read-only).");
                    ...
                    // Disabled in read-only branch:
                    // Memory.WriteBufferEnsure(invokeHookFunction, invokeHookBytes.AsSpan());
                    LoneLogging.WriteLine("[NativeHook]: Skipping writing final invoke hook shellcode (read-only).");
                    // Success -> Set Cache
                    CodeCave = codeCave;
                    SetCache();
                    LoneLogging.WriteLine("[NativeHook]: Initialize() -> OK (read-only mode, writes disabled)");
                    return true;
                }
                catch (Exception ex)
                {
                    ...
                    if (origRead)
                    {
                        for (int i = 0; i < 3; i++) // Restore bytes, try 3 times
                        {
                            try
                            {
                                ...
                                LoneLogging.WriteLine("[NativeHook]: Skipping restoration of original bytes (read-only).");
                                break;
                            }
        ...
        public static ulong? Call(ulong function, ulong rcx = 0, ulong rdx = 0, ulong r8 = 0, ulong r9 = 0)
        {
            lock (SyncRoot)
            {
                // In read-only branch, native hook calls that require memory writes are disabled.
                LoneLogging.WriteLine("[NativeHook]: Call() -> Memory writing is disabled; cannot perform call.");
                return null;
                /*
                if (!Initialized)
                {
